library(dplyr) 
rm(list = ls())
#filecluster<- "XmeanClusterbyDM_solo_specie_valutate.csv"
filecluster<-"Xmean_cluster_wo_Aloc_TR.csv"
...
df1 <- read.csv(file=paste0("./",filecluster), header=TRUE, sep=",")  # Dataset con i valori da filtrare
...
for(i in 1:nrow(data))
{
  if(data$specieex[i] != data$specieclu[i]){
    ...
    if(data$clusterunif[i]!="low" && data$experts[i]=="high") a <- a + 1  
    if(data$clusterunif[i]!="low" && data$experts[i]!="high") b <- b + 1    
    if(data$clusterunif[i]=="low" && data$experts[i]=="high") c <- c + 1    
    if(data$clusterunif[i]=="low" && data$experts[i]!="high") d <- d + 1 
  }
}
N <- a+b+c+d
accuracy <- (( a + d)/N)*100
...
write.csv(data, paste0("./expert_Vs_workflowxmean.csv"), row.names = F)