...
expert <-"Expert_2_assessment"
expert2 <-"Expert_ensemble_assessment"


# Upload both files
df2 <- read.csv(file=paste0("./",expert,".csv"), header=TRUE, sep=",") 
#df1 <- read.csv(file=paste0("./VAE/",filecluster,".csv"), header=TRUE, sep=",")  
df1 <- read.csv(file=paste0("./",expert2,".csv"), header=TRUE, sep=",") 
...
data <- cbind(df2_ordered[, c("specie", "commonness_from_experts")], 
              ...
              df1_ordered[, c("specie", "commonness_from_experts")])
...
A_0_B_1=rbind(comparisondf[which(comparisondf$A=="low" & comparisondf$B=="high"),],
...
cat("\nThree classes:\n")
A_2_B_2=rbind(comparisondf[which(comparisondf$A=="high" & comparisondf$B=="high"),])
A_2_B_1=rbind(comparisondf[which(comparisondf$A=="high" & comparisondf$B=="medium"),])
A_2_B_0=rbind(comparisondf[which(comparisondf$A=="high" & comparisondf$B=="low"),])

A_1_B_2=rbind(comparisondf[which(comparisondf$A=="medium" & comparisondf$B=="high"),])
A_1_B_1=rbind(comparisondf[which(comparisondf$A=="medium" & comparisondf$B=="medium"),])
A_1_B_0=rbind(comparisondf[which(comparisondf$A=="medium" & comparisondf$B=="low"),])


A_0_B_2=rbind(comparisondf[which(comparisondf$A=="low" & comparisondf$B=="high"),])
A_0_B_1=rbind(comparisondf[which(comparisondf$A=="low" & comparisondf$B=="medium"),])
A_0_B_0=rbind(comparisondf[which(comparisondf$A=="low" & comparisondf$B=="low"),])
...
write.csv(data, paste0("./Clustering/",expert,"_Vs_",expert2,".csv"), row.names = F)
